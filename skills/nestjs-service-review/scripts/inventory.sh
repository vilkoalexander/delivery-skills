...
while IFS= read -r f; do
  ...
  [ -f "$d/$b.spec.ts" ] || [ -f "$d/__tests__/$b.spec.ts" ] || {
    printf '  %s\n' "${f#$app/}"; missing=1
  }
done < <(find "$app" \( -name '*.service.ts' -o -name '*.controller.ts' \) | sort)
[ "$missing" -eq 0 ] && echo "  none"

echo
echo "=== guards / interceptors / decorators in use"
grep -rhoE '@(UseGuards|UseInterceptors)\([^)]*\)' "$app" --include='*.controller.ts' 2>/dev/null | sort | uniq -c | sort -rn | sed 's/^/  /'
...
if [ -n "$tu" ]; then
  ...
  grep -rhoE 'export (async )?(function|const) [A-Za-z0-9_]+' "$tu" 2>/dev/null \
    | sed -E 's/^export (async )?(function|const) //' | sort -u | sed 's/^/  /'
fi
